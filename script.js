...
class RenderToDos {
    ...
    deleteEvent(id) {
        ...
        events = events.filter(el => el.id !== Number(id))
        .sort(function (a, b) { return a.start - b.start });
    ...
    renderToDoTask(task) {
        ...
        actContainer.innerHTML = `
            ...
            <div class='buttons'>
                ...
                <button class="task-btn delete-btn" title="Delete"><i class="fa-solid fa-xmark icons"></i></button>
            </div>
        `
        ...
        const deleteBtn = actContainer.querySelector(".delete-btn");

        deleteBtn.onclick = (e) => {
            events = events.filter(el => el.start !== task.start && el.title !== task.title)
            .sort(function (a, b) { return a.start - b.start });
            this.toDoList = new ToDosList(events);
